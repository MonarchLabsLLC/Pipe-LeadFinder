...
export async function GET(req: NextRequest) {
  ...
  if (!owner) {
    ...
    if (byEmail && !byEmail.keycloakSubId) {
      ...
      // A workspace must bind in place to an owner tenant that already exists
      // in this application. Never create a tenant as a side effect of a
      // member launch: it could turn an upstream identity mismatch into an
      // apparently valid but empty workspace.
      return new NextResponse("Workspace owner has not provisioned LeadFinder", {
        status: 409,
      })
    } else {
