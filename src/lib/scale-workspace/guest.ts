...
import {
  ...
  isScaleWorkspaceOwnerUsageEnabled,
} from "./hub"
import {
...
/**
 * Shared-data access and owner-billed work have separate rollout approvals.
 * Route handlers call this only immediately before an operation that charges
 * the bound owner. Personal requests remain unchanged.
 */
export function denyUnapprovedOwnerUsage(
  scope: WorkspaceScopeOk
): NextResponse | null {
  if (!scope.isGuest || isScaleWorkspaceOwnerUsageEnabled()) return null
  return NextResponse.json(
    {
      error: "Owner-billed workspace operations are not enabled yet.",
      code: "WORKSPACE_OWNER_USAGE_DISABLED",
    },
    { status: 403 }
  )
}
