...
/**
 * Owner-billed guest work is a separate product decision from basic shared
 * workspace access. Keep searches, enrichment, and scoring unavailable to
 * guests until it is explicitly approved and enabled on the server.
 */
export function isScaleWorkspaceOwnerUsageEnabled(): boolean {
  return process.env.SCALE_TEAM_WORKSPACES_OWNER_USAGE_ENABLED === "true"
}
