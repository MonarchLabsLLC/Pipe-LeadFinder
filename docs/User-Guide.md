...
**Talk to the agent.** With Pro Max, **Lead Search → New Search** opens on a big "Who do you want to find?" box. Type who you're after in your own words — "dentists in Tampa, I need their emails", "HR managers in Chicago, 10 results" — or tap **Find people**, **Find local businesses**, **Find companies** or **Find creators** first. Suggested prompts sit under the box and your recent conversations are listed below them. The five search cards and your recent searches stay further down under **Or choose a search yourself**.

**Quick questions.** When a detail is missing (the city, how many results, which list to save to, emails or phones) the agent asks one short question with tap-to-answer buttons. Tap an answer, or choose **Something else** and type your own. It asks at most a few questions and never asks about something you already said.

**Approval cards.** Before anything costs credits or changes your data the agent shows a card: the search type, the criteria, the most it can cost ("Up to 125 (5 × 25)") and the list it saves to — a new list is marked **New** and is created only when you approve. **Approve & run** starts it, **Edit** opens the matching search form already filled in, **Reject** drops it. Cards expire after 15 minutes; if prices or the records change first you are asked to prepare a fresh one.

**Results and next steps.** When an approved search finishes, a results card shows how many leads were found, how many have an email or phone and a few examples, with **Open list**, **Export CSV** and one-tap follow-ups: **Enrich the ones without email**, **Score them** and **Send to PipeLeads CRM**. Each follow-up ends in its own approval card. You can also ask the agent to apply or remove a label, add leads to a MailBaser list, run a recent search again, show your credit balance and prices, or save a search as a daily, weekly or monthly AI Agent (it runs on its own schedule after you approve, until you pause it).

**Anywhere in the app.** **Agent** in the header opens the same conversation docked on the right; on a wide screen it moves the page over instead of covering it. Under **Context** you can pick one saved list (the agent then works only with that list) and exact leads for enrichment or scoring. The history button reopens earlier conversations and **+** starts a new one. Long jobs show progress; you can close the panel and come back. Failed or uncertain paid jobs are never repeated automatically.
...
**Connect to Superpowers** (under **Context**) opens the private Codex/Claude installation and ClickCampaigns OAuth guide. No customer API key is required. Sharing your selected list is optional; external conversations remain in Codex or Claude. See [Focused Agent operations](Focused-Agent.md) and [Agent-native design](AGENT-NATIVE.md) for rollout details.
