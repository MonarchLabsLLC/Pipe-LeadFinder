...
Native endpoints live under `/api/focused-agent`; service endpoints implement `/api/godmode/service/v1`. The bounded action registry is shared by both surfaces. The MCP service (`SERVICE_ACTIONS`, advertised by `GET capabilities`) exposes every registry tool except `ask_user`: `list_resources`, `get_list`, `get_run`, `get_crm_destinations`, `get_crm_transfer_status`, `interpret_request`, `get_credits`, `list_recent_searches`, `list_labels`, `get_export_link`, `get_handoff_options`, and the approval-gated `prepare_search`, `prepare_rerun_search`, `prepare_enrichment`, `prepare_bulk_enrichment`, `prepare_scoring`, `prepare_label_change`, `prepare_handoff`, `prepare_scheduled_agent` and `prepare_crm_transfer`. A `prepare_*` call only creates an immutable, expiring proposal (with its `approvalUrl`); only `POST proposals/:id/execute` with the signed human approval grant runs it. Each request checks authoritative ClickCampaigns Pro Max entitlement and current destination-app access. Verified Keycloak subjects resolve to existing local users only; email matching, auto-provisioning, development login and unverified token decoding are not accepted by this Agent. Existing sessions must sign in again to obtain verified provenance.
